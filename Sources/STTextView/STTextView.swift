...
open class STTextView: NSView, NSTextInput, NSTextInputClient {
    ...
    // NSTextInputClient implementation need to stay here, not in the extension!
    // due to compiler issue where it can't build it properly (Swift 5.8)
    // https://github.com/krzyzanowskim/STTextView/discussions/18
    ...
    @objc(unmarkText)
    @_implements(NSTextInputClient, unmarkText())
    public func textInputClient_unmarkText() {
        if !hasMarkedText() {
            return
        }
    ...
    @objc(markedRange)
    @_implements(NSTextInputClient, markedRange())
    public func textInputClient_markedRange() -> NSRange {
        markedText?.replacementRange ?? .notFound
    }

    /// Returns whether or not the receiver has marked text.
    @objc(hasMarkedText)
    @_implements(NSTextInputClient, hasMarkedText())
    public func textInputClient_hasMarkedText() -> Bool {
    ...
    public func textInputClient_attributedString() -> NSAttributedString {
        textContentManager.attributedString(in: nil) ?? NSAttributedString()
    }
    ...
    @_implements(NSTextInputClient, characterIndex(for:))
    public func textInputClient_characterIndex(for point: NSPoint) -> Int {

...
//        return CGRect(x: minX, y: minY, width: maxX, height: maxY - minY)
    ...
    public func textViewportLayoutControllerWillLayout(_ textViewportLayoutController: NSTextViewportLayoutController) {
        ...
        for subview in contentView.subviews/*.filter({ type(of: $0) != STInsertionPointView })*/ {
            subview.removeFromSuperview()
        }
    }

    public func textViewportLayoutController(_ textViewportLayoutController: NSTextViewportLayoutController, configureRenderingSurfaceFor textLayoutFragment: NSTextLayoutFragment) {
        let fragmentView = fragmentViewMap.object(forKey: textLayoutFragment) ?? STTextLayoutFragmentView(layoutFragment: textLayoutFragment, frame: .zero)
        // Adjust position

...
public struct TextViewOptions: OptionSet, Sendable {
    ...
    public static let disableAutocorrection = TextViewOptions(rawValue: 1 << 3)

    /// Disable smart quote substitution (e.g., " to "").
    public static let disableSmartQuotes = TextViewOptions(rawValue: 1 << 4)

    // MARK: - iOS only options

    /// Disable automatic capitalization.
    @available(macOS, unavailable)
    public static let disableAutocapitalization = TextViewOptions(rawValue: 1 << 5)

    /// Disable smart dash substitution (e.g., -- to —).
    @available(macOS, unavailable)
    public static let disableSmartDashes = TextViewOptions(rawValue: 1 << 6)

    /// Disable smart insert/delete behavior.
    @available(macOS, unavailable)
    public static let disableSmartInsertDelete = TextViewOptions(rawValue: 1 << 7)
    ...
    public static let disableTextReplacement = TextViewOptions(rawValue: 1 << 8)

    /// Disable automatic text completion.
    @available(iOS, unavailable)
    @available(macCatalyst, unavailable)
    public static let disableTextCompletion = TextViewOptions(rawValue: 1 << 9)
}
